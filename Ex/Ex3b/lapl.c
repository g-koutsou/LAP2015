...
#pragma omp parallel for
  for(int y=0; y<L/4; y++)
	...
	u1m0i[iy] = u[vm0*8*ND+8+4+iy];
      ...
      /*
	If on left boundary (-y), shift elements up
       */
      if(y == 0)
	for(int iy=3; iy>0; iy--) {
      ...
      /*
	If on right boundary (+y), shift elements down
       */      
      if(y == L/4-1)
	for(int iy=0; iy<3; iy++) {
	  ...
	  /*
	    ...
	    reorder entries of boundary element
	   */
	}
	...
	*/
	...
	/* 
	   ...
	   complete imaginary part of result... 
	*/
      }
    }
  return;
...
cg_folded(int L, float *x, float *b, float *u)
{
  ...
  /*
    ...
    Find number of Flops and Bytes of I/O and devide by t_lapl to
    obtain FP- and IO-rates    
   */
  double beta_fp = 0;
  double beta_io = 0;
  printf(" Converged after %6d iterations, res = %+e\n", iter, rr/bb);  
